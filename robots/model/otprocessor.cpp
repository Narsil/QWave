...
void OTProcessor::submitNext()
{
    ...
    {
        if ( op.hasMutation() )
            emit documentMutation(op.documentId(), op.mutation(), m_outgoingDeltas[0].author());
    ...
    qDebug()<<"VERSION "<<m_outgoingDeltas[0].version().version;
    qDebug()<<"HASH "<<m_outgoingDeltas[0].version().hash;
